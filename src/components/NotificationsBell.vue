...
<template>
  <div ref="rootRef" class="notifications-bell">
    <button type="button" class="notifications-bell-trigger" :aria-expanded="isOpen" aria-haspopup="true"
      aria-label="Уведомления" @click.stop="toggleDropdown">
      <svg class="notifications-bell-icon" viewBox="0 0 24 24" aria-hidden="true">
        <path
          d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.63-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.64 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2zm-2 1H8v-6c0-2.48 1.51-4.5 4-4.5s4 2.02 4 4.5v6z" />
      </svg>
      <span v-if="hasUnread" class="notifications-bell-badge" aria-hidden="true" />
    </button>
    ...
    <div v-if="isOpen" class="notifications-dropdown card" @click.stop>
      ...
      <p v-else-if="notifications.length === 0" class="notifications-dropdown-message">
        Уведомлений пока нет.
      </p>

      <ul v-else class="notifications-list">
        <li v-for="notification in notifications" :key="notification.id" class="notifications-list-item"
          :class="{ unread: !notification.is_read }" @mouseenter="
            onNotificationMouseEnter(notification.id, notification.is_read)
            ">
          <span v-if="!notification.is_read" class="notifications-unread-dot" aria-hidden="true" />
          <div class="notifications-list-content">
            <p class="notifications-text">{{ notification.body }}</p>
            <p class="meta notifications-time">
              {{ formatDateTime(notification.created_at) }}
            </p>
            <RouterLink :to="`/chats/${notification.chat_id}`" class="notifications-chat-link" @click="isOpen = false">
